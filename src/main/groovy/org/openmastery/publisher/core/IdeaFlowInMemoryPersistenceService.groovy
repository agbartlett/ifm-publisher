...
public class IdeaFlowInMemoryPersistenceService implements IdeaFlowPersistenceService {
	...
	@Override
	Page<TaskEntity> findRecentTasks(Long userId, Pageable pageable) {
		if (pageable.getPageSize() < 1) {
			return []
		} else if (pageable.getPageSize() >= taskList.size()) {
			return taskList.asImmutable()
		} else {
			List<TaskEntity> sortedList = taskList.toSorted { a, b -> b.modifyDate <=> a.modifyDate }
			return sortedList.subList(0, pageable.getPageSize())
		}
	}
